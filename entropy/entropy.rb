...
class Entropy
  def self.configure(config, settings)
    ...
    entropy_dir = File.dirname(__FILE__)
    ...
    config.vm.provision 'shell' do |s|
      s.path = entropy_dir + '/clear-apache.sh'
    end
    ...
    if settings.include? 'sites'
      settings['sites'].each do |site|
        ...
        config.vm.provision 'shell' do |s|
          ...
          if type == "apache"
            s.path = entropy_dir + "/serve-#{type}.sh"
          else
    ...
    config.vm.provision 'shell' do |s|
      ...
      s.path = entropy_dir + '/clear-variables.sh'
    end

    if settings.has_key?('variables')
      settings['variables'].each do |var|
        config.vm.provision 'shell' do |s|
          s.inline = "echo \"\nenv[$1] = '$2'\" >> /etc/opt/remi/php56/php-fpm.d/www.conf"
        ...
        config.vm.provision 'shell' do |s|
            s.inline = "echo \"\nenv[$1] = '$2'\" >> /etc/opt/remi/php73/php-fpm.d/www.conf"
            s.args = [var['key'], var['value']]
        end
      ...
      config.vm.provision 'shell' do |s|
        s.inline = 'sudo systemctl restart php56-php-fpm; sudo systemctl restart php70-php-fpm; sudo systemctl restart php71-php-fpm; sudo systemctl restart php72-php-fpm; sudo systemctl restart php73-php-fpm;'
      end
    end
    ...
    if settings.has_key?('blackfire')
      config.vm.provision 'shell' do |s|
        s.path = entropy_dir + '/blackfire.sh'
        s.args = [
